...
export class MarkupWritingComponent implements OnInit, OnChanges, DoCheck {
  ...
  async addPageBreaks(markup){
    ...
    for (var i = 0; i < markup.children.length; i++) {
        ...
        this.elrows = 0;
        //console.log(rows);
        //append textarea hard returns to totalrows
        // for(let j=0; j < rows; j++){
        //   totalrows += 1;
        //   elrows += 1;
        //   if (totalrows % this.rowsperpage === 0){
        //     this.createPageBreak(el,elrows);
        //   }
        // }
        ...
        for(let i = 0,len = rows; i < len; i++){
          ...
          //totalrows += softreturns;
          // for(let k=0; k < softreturns; k++){
          //   totalrows += 1;
          //   elrows += 1;
          //   console.log(totalrows);
          //   if (totalrows % this.rowsperpage === 0){
  ...
  async createPageBreak(el){
    let divider = document.createElement("hr");
        ...
        //el.children.length > 4 ? el.childNodes[0] = divider : el.prepend(divider);
        el.prepend(divider);
  }
