...
export class MarkupWritingComponent implements OnInit, OnChanges, DoCheck {
  ...
  @ViewChildren('eltextarea', {read: ElementRef })  textElements: QueryList<ElementRef>;
  ...
  @HostListener('keydown', ['$event']) keydownReturn(event: KeyboardEvent) {
    if(event.shiftKey && event.keyCode == 8){
      console.log(this.currentseggroup.id);
      if((this.currentseggroup.id-1) >= 0){
        console.log(this.currentel);
        this.editElement('delete',this.currentel,this.currentel.id,this.currentseggroupid);
        const sgidx = (this.currentseggroupid -1) > 0 ? (this.currentseggroupid -1) : 0;
  ...
  focusElement(eidx){
    ...
    setTimeout(()=>{
      ...
      let arr:any = [];
      if(element.nodeName == 'INPUT') arr = this.inputElements;
      if(element.nodeName == 'ION-TEXTAREA') arr = this.textElements;
      if(arr.length){
        arr.toArray().forEach((item,index) => {
            let classes = item.nativeElement.classList.toString();
            let match = 'sgidx-'+sgidx+' sidx-'+sidx+' eidx-'+eidx;
            item.nativeElement.classList.add('hydrated2');
            console.dir(item.nativeElement);
            if( classes.includes(match) ) item.nativeElement.setFocus();
        });
      }
    });
  }
  ...
  dragElement(sgidx, sidx, event: CdkDragDrop<string[]>) {
    this.selectSegmentGroup(sgidx);
    this.selectSegment(sidx);
    let elements = this.currentseggroup.segments[this.currentsegid].elements;
 ...
 async importMarkup_PDF(){
  ...
  async newSegment(){
    ...
    //g.segments[this.maxsegmentid].elements = newsegment;
    g.segments[this.maxsegmentid].params.name = newsegmentdata.fields.name.value;
    this.resetSegment(); 
    this.ref.detectChanges();
  }
  ...
  dragSegmentGroup(event: CdkDragDrop<string[]>) {
