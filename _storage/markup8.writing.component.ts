...
export class MarkupWritingComponent implements OnInit, OnChanges, DoCheck {
  ...
  async converttoHTML(){
    ...
    this.markup.forEach(async el=>{
      switch(el.key){
        case 's':
          html += '<div class="el_s">' + this.dividePages(el.value.name.value + '\n\n') + '</div>';
        break;
        case 'c':
        ...
        break;
        case 'body':
          html += '<div class="el_b">' + this.dividePages(el.value + '\n\n') + '</div>';
        break;
        case 'dialogue':
          html += '<div class="el_d">' + el.value + '\n\n</div>';
        break;
      }
    });
    console.log(html);
  ...
  createDivide(hr,rows){
    ...
    words.forEach((word, idx) => {
      ...
      lines +=  pbreak + lbreak2 + word + ' ' + lbreak;
      //lines +=  lbreak + word + ' ';
